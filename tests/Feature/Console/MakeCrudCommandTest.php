...
PHP);
    ...
    try {
        ...
        expect(File::get(base_path('app/Models/RootSqlWidget.php')))
            ...
            ->and(File::get(base_path('app/Http/Controllers/RootSqlWidgetController.php')))
            ->toContain('...$rootSqlWidget->toArray(),')
            ->and(substr_count(File::get(base_path('app/Http/Controllers/RootSqlWidgetController.php')), '...$rootSqlWidget->toArray(),'))->toBe(3)
            ->and(File::get(base_path('app/Http/Controllers/RootSqlWidgetController.php')))
            ->not->toContain("'id' => \$rootSqlWidget->id,")
            ->not->toContain("'name' => \$rootSqlWidget->name,")
            ...
            ->toContain('Create rootSqlWidget')
            ->not->toContain('root-sql-widgetsIndex')
            ->and(File::get(base_path('resources/js/pages/root-sql-widgets/Create.vue')))
            ->toContain('setLayoutProps')
            ->toContain("t('Create :name', { name: t('rootSqlWidget') })")
            ->and(File::get(base_path('resources/js/pages/root-sql-widgets/Edit.vue')))
            ->toContain('setLayoutProps')
            ->toContain("t('Edit :name', { name: t('rootSqlWidget') })")
