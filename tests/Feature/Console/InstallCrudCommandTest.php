...
test('crud frontend resources expose the paginator contract', function () {
    ...
    expect($component)
        ...
        ->toContain('hiddenColumns?: string[];')
        ->toContain('hiddenFields?: string[];')
        ->toContain('v-if="workspace"')
        ->toContain('canShowRecord')
        ->toContain('show.href(record)')
        ->toContain('font-bold')
        ->toContain('filtersOpen')
        ->toContain('fieldsAfter')
        ->toContain(':fields-after="fieldsAfter"')
        ->toContain(':values="slotProps.values"')
        ->toContain('fullPageForms')
        ->toContain('next_page_url')
        ->toContain('prev_page_url')
        ->toContain('hasPaginationControls')
        ->not->toContain('v-if="records.last_page > 1"')
        ->toContain('#toolbar')
        ->toContain('schema.search?.enabled || visibleFilters.length > 0')
        ->toContain(':columns="visibleColumns"')
        ->toContain(':fields="visibleFields"')
        ->toContain(':fields="visibleUpdateFields"')
        ->and($table)
        ->toContain('md:hidden')
        ...
        ->toContain('multiple')
        ->toContain('tabindex="0"')
        ->and(File::get(dirname(__DIR__, 3).'/resources/js/components/crud/RemoteCombobox.vue'))
        ->toContain('tabindex="0"')
        ->and($selectMultiple)
        ...
        ->toContain('filters_open: boolean;')
        ->toContain('last_page?: number;')
        ->toContain('total?: number;')
        ->toContain('next_page_url: string | null;')
        ->toContain('prev_page_url: string | null;')
        ->toContain("| 'combobox'")
...
test('crud frontend resources expose the translation helper and catalog', function () {
    ...
    expect($composable)
        ...
        ->toContain("new RegExp(`:${placeholder}(?![A-Za-z0-9_])`, 'g')")
        ->and($translations)
        ->toContain('"Add": "Add"')
        ->and($translations)
